...
public abstract class StatsUsage {
	...
	public static final String OBJECT_POPULARITY_QUERY = "SELECT * FROM (SELECT row_number() OVER() as rid, a.OBJECT_ID as subject, SUM(a.NUM_ACCESS) as num_access, SUM(a.NUM_VIEW) as num_view FROM WEB_STATS s, STATS_DLP_OBJECT_ACCESS a WHERE s.ID=a.STAT_ID AND s.APP_NAME=? GROUP BY a.OBJECT_ID ORDER BY num_view DESC, num_access DESC) as result WHERE result.rid<=150"; 
