	...
	public static void listFile(Map<String, File> fMap, File file) throws Exception{
		...
		for(int i=0; i<files.size(); i++){
			...
			if(fMap.get(fName) != null){
				...
				if(file.getAbsolutePath().indexOf("/siogeocoll/Master_Files_Bag/") > 0 || fName.equalsIgnoreCase("Thumbs.db") || fName.equals(".DS_Store") || ((fName.indexOf("validation") >=0 || fName.indexOf("manifest") >= 0 || fName.startsWith("bagit") || fName.startsWith("bag-info")) && fName.endsWith(".txt")))
					log.warn(message);
				else
