...
<script type="text/javascript">
    ...
    function doSelect(obj){
        ...
        if (html === 'Select All') {
            $('input[type="checkbox"]').each(function() {
                ...
                else if ($(this).attr('name') != 'excludeComponents') {
            ...
            $('input[type="checkbox"]:checked').each(function() {
                if ($(this).attr('name') != 'excludeComponents') {
                    $(this).attr('checked', false);
                }
             });
             $(obj).html('Select All');
    ...
    function batchExport(formID) {
        if (formID == 'records') {
            ...
            if (inputVal.trim().length == 0) {
                if (sourceName == 'file')
                    alert ("Please choose a file containing the records.");
                else
            ...
            var exeConfirm = confirm('Are you sure to export records in ' + inputVal + '? \n');
            if (exeConfirm) {
                formObj.action = "/damsmanager/operationHandler.do?batchExport&progress=0&formId=records&sid=" + getSid();
            ...
            if (collections.length===0) {
                alert('Please check a collection!');
                return false;
            }
            var exeConfirm = confirm('Are you sure to export metadata the following ' + count + ' collections? \n' + collectionNames);
            if (exeConfirm) {
                mainForm.category.value = collections;
                document.mainForm.action = "/damsmanager/operationHandler.do?batchExport&progress=0&formId=mainForm&sid=" + getSid();
                displayMessage("message", "");
    ...
    var crumbs = [{"Home":"http://libraries.ucsd.edu"}, {"Digital Library Collections":"/curator"},{"DAMS Manager":"/damsmanager/"}, {"Batch Export":""}];
    drawBreadcrumbNMenu(crumbs, "tdr_crumbs_content", true);

    $(document).ready(function() {
        $('input:radio[name="exportFormat"]').change(
            function() {
                if ($(this).is(':checked') && $(this).val() != 'csv') {
                    $("input[name='excludeComponents']:checkbox").removeAttr('checked');
                    $("input[name='excludeComponents']:checkbox").attr('disabled', true);
                } else {
                    $("input[name='excludeComponents']:checkbox").attr('disabled', false);
                }
            });
        });
</script>
...
<div id="main" class="gallery" align="center">
  <div style="font-size:24px;font-weight:bold;color:#336699;margin-bottom:10px;margin-top:10px;">Batch Export</div>
  <div id="tabs" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
    ...
    <div id="recordsTab" class="ui-tabs-panel ui-widget-content ui-corner-bottom" style="display:none">
      <form id="records" name="records" method="post" enctype="multipart/form-data">
        <table style="margin-bottom:10px;text-align:left;margin:30px;">
          ...
          <tr>
            ...
            <td  align="left">
                <div class="submenuText">
                    <div id="text"><input type="text" id="textInput" name="textInput" size="50" value=""><div class="note"> (Comma-separated list of ARKs)</div></div>
          ...
          <tr>
            <td height="30px" align="left" colspan="2">
                ...
                <div class="formatOptions">
                    <div title="Check this checkbox to exclude metadata in the components and files."><input type="checkbox" id="excludeComponents" name="excludeComponents">
                        <span class="text-special">Exclude components and files.</span>
                    </div>
                <div class="formatOptions">
            </td>
          </tr>
        ...
        <div class="buttonDiv">
          <input type="button" name="buttonSubmit" value="Submit" onClick="batchExport('records');"/>
        </div>
      </form>
    </div>

    <div id="colsTab" class="ui-tabs-panel ui-widget-content ui-corner-bottom">
    <form id="mainForm" name="mainForm" method="post" action="/damsmanager/operationHandler.do?batchExport" >
                        ...
                        <c:forEach var="item" items="${model.collections}" varStatus="status">
                            <c:if test="${(status.count-1)%2==0}">
                                <li><input type="checkbox" name="collectionId" value="${item.subject}"/><span id="${item.subject}" class="listTitle">${item.title}</span>
                                    <c:if test="${item.count!='0'}"><span title="Records found: ${item.count}" style="cursor:pointer;color:gray;"><span class="listTitle">(</span><span class="count">${item.count}</span><span class="listTitle">)</span></span></c:if>
                                </li>
                            </c:if>
                        </c:forEach>
                    </ul>
    ...
    <div style="text-align: left;">
        ...
        <div class="formatOptions">
            <div title="Check this checkbox to exclude metadata in the components and files."><input type="checkbox" name="excludeComponents">
                <span class="text-special">Exclude components and files.</span>
            </div>
        </div>
    </div>
    ...
    <div class="buttonDiv">
      ...
      <input type="button" name="export" value="Export" onClick="batchExport('mainForm');"/>&nbsp;&nbsp;
    </div>
    </form>    
    </div>
  </div>
