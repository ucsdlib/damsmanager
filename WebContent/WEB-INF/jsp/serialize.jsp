...
<script type="text/javascript">
	...
	function reloadPage(){
		...
		document.location.href="/damsmanager/serialize.do?ts=" + ds;
	}
	...
	function serialize(){
		...
		if(collections.length===0){
			alert('Please check a collection for filestore serialization!');
			return false;
		}
	    var exeConfirm = confirm('Are you sure to serialize the following ' + count + ' collection(s) to filestore? \n' + collectionNames);
	    if(exeConfirm){
		    mainForm.category.value = collections;
	    	document.mainForm.action = "/damsmanager/operationHandler.do?serialize&progress=0&formId=mainForm&sid=" + getSid();
	...
	var crumbs = [{"Home":"http://libraries.ucsd.edu"}, {"Digital Library Collections":"/curator"},{"DAMS Manager":"/damsmanager/"}, {"Serialize":""}];
...
<div id="main" class="gallery" align="center">
	<form id="mainForm" name="mainForm" method="post" action="/damsmanager/operationHandler.do?serialize" >
	<div style="font-size:24px;font-weight:bold;color:#336699;margin-bottom:10px;margin-top:10px;">RDF/XML Filestore Serialization</div>
...
<div class="buttonDiv">
	...
	<input type="button" name="update" value="Serialize" onClick="serialize();"/>&nbsp;&nbsp;
</div>
</div>
</form>
</div>
