...
<script type="text/javascript">	
	function confirmIngest(){
    	...
    	document.mainForm.action = "/damsmanager/operationHandler.do?ds=" + ds + "&jetl&progress=0&formId=mainForm&sid=" + getSid();
	...
	function selectCollection(selectObj){
		...
		if (selectedIndex == 0) {
			document.mainForm.action = "/damsmanager/jetl.do?ds=" + ds;
			document.mainForm.submit();
		}
		else {
			var collectionId = selectObj.options[selectedIndex].value;
			document.mainForm.action = "/damsmanager/jetl.do?ds=" + ds + "&reset&collection=" + collectionId;
			document.mainForm.submit();
		}
	}
	...
	function reloadPage(){
		var dsIdx = document.mainForm.ds.selectedIndex;
		var ds = document.mainForm.ds.options[dsIdx].value;
		document.location.href="/damsmanager/jetl.do?ds=" + ds;
	}
</script>
...
<form id="mainForm" name="mainForm" method="post" action="/damsmanager/operationHandler.do?jetl" >
<div class="adjustDiv" style="width:850px">
<div class="emBox_ark">
<div class="emBoxBanner">JETL Loading</div>
<div style="background:#DDDDDD;padding-top:8px;padding-bottom:8px;padding-left:25px;" align="left">
		<span id="dsSpan" class="menuText" Title="Double click to change the triplestore used for operation." ondblclick="setTriplestore();" onMouseOver="this.style.cursor='pointer'">${fn:toUpperCase(tsNameFl)}${fn:substring(model.triplestore, 1, tsNameLen)} </span>
			...
			<span><select id="collection" name="collection" onChange="selectCollection(this);" class="inputText" >
						<option value=""> -- collections -- </option>
						<c:forEach var="entry" items="${model.collections}">
							<option value="${entry.value}" <c:if test="${model.collectionId == entry.value}">selected</c:if>>
...
<div style="margin-top:15px;">
	 <fieldset class="groupbox_ark"><legend class="slegandText">Options</legend>
		 <div title="Check this checkbox to upload the master files to SRB without JHOVE extraction." class="specialmenuText">
					<input type="checkbox" name="masterFile" class="pcheckbox">Upload master files to SRB only ( No JHOVE data)
		 </div>
	  </fieldset>
</div>
 -->
